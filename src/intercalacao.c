...
int compare_records(const void *a, const void *b, TipoRegistro tipo) {
    switch (tipo) {
        ...
        case LIVRO:
            return ((TLivro*)a)->id - ((TLivro*)b)->id;
        default:
...
void intercalacao_otima_Livro() {
    ...
    while (1) {
        ...
        // Encontrar o menor registro entre as partições
        for (int i = 0; i < NUM_PARTITIONS; i++) {
            if (records[i].id != INT_MAX && (minIndex == -1 || compare_records(&records[i], &minRecord, LIVRO) < 0)) {
                minIndex = i;
                minRecord = records[i];
            }
        }
...
void intercalacoes_otimas() {
    ...
    e = save_intercalacao_otima_struct(NUM_PARTITIONS, 100, cpu_time_used);
