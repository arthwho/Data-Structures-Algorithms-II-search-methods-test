...
int classificacao_interna_Func(int M) {
    ...
    while (reg != nFunc) {
        ...
        for (int j = 0; j < M / 2; j++) {
            ...
            printf("Debug: Seleção Natural Funcionarios Finalizado\n");
        }
        ...
        if ((q = fopen(nomeParticao, "wb+")) == NULL) {
            printf("Erro ao criar arquivo de saída\n");
        } else {
            for (int i = 0; i < M; i++) {
                fseek(q, (i) * t, SEEK_SET);
                fwrite(v[i], sizeof(TFunc), 1, q);
            }
            fclose(q);
            qtdParticoes++;
        }
    ...
    //printf("Debug 2: Classificação Interna Funcionarios Finalizado\n");
    }
...
int classificacao_interna_Livro(int M) {
    ...
    while (reg != nLivros) {
        ...
        for (int j = 0; j < M / 2; j++) {
            ...
            int maxIndex = j;
            for (int k = j + 1; k < M - j; k++) {
                if (v[k]->id < v[minIndex]->id) {
                    minIndex = k;
                } else if (v[k]->id > v[maxIndex]->id) {
                    maxIndex = k;
                }
            }
            if (minIndex != maxIndex) {
            ...
            printf("Debug: Seleção Natural Livros Finalizado\n");
        }
